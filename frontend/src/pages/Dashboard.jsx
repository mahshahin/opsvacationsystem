      ...
      <div className="p-4 md:p-8">
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 relative">
          ...
          <form
            ...
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full items-start relative z-20"
          >
            <div className="relative z-30 w-full">
              ...
              <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-400">
                <ChevronDown
                  size={18}
                  className={`transition-transform duration-300 ${isLeaveMenuOpen ? "rotate-180" : ""}`}
                />
              </div>
                    ...
                    {["annual", "casual", "compensation"].map((type) => (
                      <div
                        ...
                        className={`px-4 py-3 cursor-pointer transition-colors flex items-center gap-2 ${
                          leaveType === type
                            ? "bg-blue-50/60 text-navy-light font-bold"
                            : "text-gray-600 hover:bg-gray-50"
                        }`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${leaveType === type ? "bg-navy-light" : "bg-transparent"}`}
                        ></div>
                        {type === "annual"
                          ? "إجازة اعتيادية"
                          : type === "casual"
            ...
            <div className="w-full">
              <button
                ...
                className="w-full h-full py-3 flex items-center justify-center gap-2 bg-navy-light text-white rounded-xl font-bold hover:bg-[#0f172a] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 z-10"
              >
                {isSubmitting ? (
                ...
                {myRequests.length > 0 ? (
                  myRequests.slice(0, 10).map((req) => (
                    <tr key={req._id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-4 text-sm font-bold text-navy-light text-center">
                        {req.duration} يوم
                      </td>
