...
const CircularProgress = ({ value, max, label, type }) => {
  ...
  // 🌟 التعديل السحري هنا: استخدمنا Math.min عشان النسبة مستحيل تزيد عن 100%، وMath.max عشان مستحيل تقل عن صفر
  ...
  const config = {
    annual: {
      ...
      icon: <Umbrella className="w-3 h-3 md:w-5 md:h-5 text-blue-500 mb-0 md:mb-1" />
    },
    casual: {
      gradient: ['#f59e0b', '#b45309'], 
      shadow: 'drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]',
      bgClass: 'bg-yellow-50/50',
      icon: <AlertCircle className="w-3 h-3 md:w-5 md:h-5 text-yellow-500 mb-0 md:mb-1" />
    },
    compensation: {
      gradient: ['#10b981', '#047857'], 
      shadow: 'drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]',
      bgClass: 'bg-green-50/50',
      icon: <Coffee className="w-3 h-3 md:w-5 md:h-5 text-green-500 mb-0 md:mb-1" />
    }
  };
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center p-2 md:p-5 rounded-xl md:rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 overflow-hidden ${currentConfig.bgClass}`}>
      ...
      <div className="relative flex items-center justify-center mb-1.5 md:mb-3">
        <svg viewBox="0 0 160 160" className="transform -rotate-90 w-16 h-16 md:w-32 md:h-32">
          <defs>
          ...
          <circle
            cx="80" cy="80" r={radius}
            stroke={`url(#gradient-${type})`} strokeWidth="12" fill="transparent"
            strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round"
            className={`transition-all duration-1500 ease-out ${currentConfig.shadow}`}
          />
        </svg>
        ...
        <div className="absolute flex flex-col items-center justify-center mt-0.5 md:mt-0">
          ...
          <div className="flex items-baseline gap-0.5 md:gap-1">
            <span className="text-base md:text-3xl font-bold text-gray-700 tracking-tight leading-none">{value}</span>
          </div>
          <span className="text-[8px] md:text-[11px] font-semibold text-gray-400 mt-0 md:mt-0.5">يوم</span>
        </div>
      </div>
