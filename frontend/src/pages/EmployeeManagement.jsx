...
const EmployeeManagement = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('تمت إضافة الموظف بنجاح!', { style: { background: '#10b981', color: '#fff' }});
        fetchEmployees();
      ...
      <div className="p-8 bg-gray-50 min-h-screen">
        <header className="mb-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
          ...
          <div className="bg-blue-50 text-blue-800 px-4 py-2 rounded-lg font-bold text-sm">
            إجمالي القوة: {staffList.length} فرد
          </div>
        </header>
                    ...
                    <td className="p-4 text-center">
                      {emp.employeeCode === GOLDEN_ADMIN_CODE ? (
                        ...
                        <div className="flex justify-center gap-1.5">
                          ...
                          <button onClick={() => handleDeleteConfirm(emp._id, emp.name)} className="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition" title="حذف حساب الإدارة نهائياً"><Trash2 size={15} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
                    ...
                    <td className="p-4 text-center">
                      {editingId === emp._id ? (
                        ...
                        <div className="flex justify-center gap-1.5">
                          <button onClick={() => handleEditClick(emp)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition" title="تعديل البيانات"><Edit2 size={15} /></button>
                          <button onClick={() => handleResetConfirm(emp.employeeCode, emp.name)} className="p-1.5 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition" title="تصفير كلمة المرور"><RotateCcw size={15} /></button>
                          <button onClick={() => handleDeleteConfirm(emp._id, emp.name)} className="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition" title="حذف الموظف نهائياً"><Trash2 size={15} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
