...
? { username: newEmp.employeeCode.trim(), name: newEmp.name, password: '123456' }
: newEmp;
...
// 🐛 التعديل الثاني: إضافة API_URL قبل مسار التعديل
const url = isAdmin ? `${API_URL}/api/admin/update-admin/${id}` : `${API_URL}/api/admin/update-employee/${id}`;
...
toast.dismiss(t.id);
try {
// 🐛 التعديل الثالث: إضافة API_URL قبل مسار الحذف
const url = role === 'admin' ? `${API_URL}/api/admin/delete-admin/${id}` : `${API_URL}/api/admin/delete-employee/${id}`;
...
{/* حاوية السكرول الأفقي لجدول المديرين */}
...
<th className="p-4 text-center whitespace-nowrap">الصلاحية بالنظام</th>
<th className="p-4 text-center whitespace-nowrap">إجراءات التحكم الإداري</th>
</tr>
</thead>
<tbody className="divide-y divide-gray-100">
{loading ? (
<tr><td colSpan="4" className="p-8 text-center text-gray-400 whitespace-nowrap">جاري تحميل البيانات...</td></tr>
) : adminsList.map(emp => (
<tr key={emp._id} className={emp.employeeCode === GOLDEN_ADMIN_CODE ? "bg-yellow-50/40" : "hover:bg-gray-50 transition"}>
<td className="p-4 text-gray-600 font-bold whitespace-nowrap">{emp.employeeCode}</td>
...
<td className="p-4 text-center whitespace-nowrap">{translateRole(emp.role)}</td>
...
{/* حاوية السكرول الأفقي لجدول الموظفين */}
<div className="w-full overflow-x-auto">
...
<th className="p-4 whitespace-nowrap">الاسم بالكامل</th>
<th className="p-4 text-center whitespace-nowrap">الدرجة</th>
<th className="p-4 text-center whitespace-nowrap">نوع العمل</th>
<th className="p-4 text-center whitespace-nowrap">الصفة بالنظام</th>
<th className="p-4 text-center whitespace-nowrap">إجراءات التحكم الإداري</th>
</tr>
</thead>
<tbody className="divide-y divide-gray-100">
{loading ? (
<tr><td colSpan="6" className="p-8 text-center text-gray-400 whitespace-nowrap">جاري تحميل البيانات...</td></tr>
) : staffList.map(emp => (
