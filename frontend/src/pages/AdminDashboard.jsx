          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  <tr><td colSpan="6" className="p-8 text-center text-gray-400">جاري تحميل الطلبات...</td></tr>
                ) : pendingRequests.length > 0 ? (
                  pendingRequests.map((req) => (
                    <tr key={req._id} className="hover:bg-gray-50 transition">
                      <td className="p-4 font-bold text-gray-800">
                        {req.employeeId?.name || 'غير معروف'}
                        <div className="text-xs text-gray-400 font-normal mt-1">كود: {req.employeeId?.employeeCode}</div>
                      </td>
                      <td className="p-4 text-sm text-gray-600">{req.employeeId?.jobGrade}</td>
                      <td className="p-4">
                        ...
                        <div className="mt-1.5">
                          <span className="inline-flex items-center gap-1 bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-[11px] font-bold border border-gray-200">
                            الرصيد المتبقي: <span className={req.employeeId?.leaveBalances?.[req.leaveType] < req.duration ? "text-red-600" : "text-gray-900"}>
                              {req.employeeId?.leaveBalances?.[req.leaveType] || 0} أيام
                            </span>
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="p-4 text-sm font-bold text-center">{req.duration} يوم</td>
                  ...
                  <tr>
                    <td colSpan="6" className="p-12 text-center text-gray-400 text-lg">
